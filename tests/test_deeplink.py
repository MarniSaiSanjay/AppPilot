...
from apppilot import email_delivery, logtags  # noqa: E402
...
class EmailReportTests(unittest.TestCase):
    ...
    def _report(self, *results):
        report = d.SuiteReport(suite_name="Deeplink")
    ...
    def test_successful_send(self):
        ...
        result, out = _capture(
            email_delivery.send_suite_report, report, env=self._ENV, transport=relay
        )
        self.assertTrue(result)
        ...
        self.assertIn("DEEPLINK TEST REPORT", body)  # full report is source of truth
        self.assertIn("Generated by AppPilot", body)  # footer
        html_body = payload["html"]
        self.assertIn("<table", html_body)
        self.assertIn(">PASS<", html_body)  # green result for the passing case
        self.assertIn(">FAIL<", html_body)  # red result for the failing case
    ...
    def test_email_failure_does_not_raise(self):
        ...
        report = self._mixed_report()
        result, out = _capture(
            email_delivery.send_suite_report, report, env=self._ENV, transport=boom
        )
        self.assertFalse(result)
    ...
    def test_success_accepts_any_2xx_status(self):
        ...
        for status in (200, 201, 202, 204):
            with self.subTest(status=status):
                ...
                result = email_delivery.send_suite_report(
                    self._mixed_report(), env=self._ENV, transport=relay
                )
                self.assertTrue(result)
    ...
    def test_oversized_body_is_truncated_before_send(self):
        # The relay caps body size; a huge suite must be truncated on the wire
        # (the console output stays the complete record). Verified via the real
        # send path so the transport-level cap is exercised, not a formatter.
        report = self._report(*[
        ...
        email_delivery.send_suite_report(report, env=self._ENV, transport=relay)
        payload = json.loads(relay.requests[0].data.decode("utf-8"))
        self.assertLessEqual(len(payload["body"]), email_delivery._MAX_BODY_CHARS + 100)
        self.assertIn("truncated", payload["body"])
...
class EmailRecipientPromptTests(unittest.TestCase):
    ...
    def _prompt(self, answers, *, interactive=True, saved=None):
        it = iter(answers)
        with mock.patch.object(email_delivery, "_load_saved_recipient", return_value=saved), \
                mock.patch.object(email_delivery, "_save_recipient") as save:
            result, out = _capture(
                email_delivery.prompt_email_recipient,
    ...
    def test_cancel_at_recipient_returns_none_not_saved_default(self):
        ...
        with mock.patch.object(
            email_delivery, "_load_saved_recipient", return_value="old@example.com"
        ), mock.patch.object(email_delivery, "_save_recipient") as save:
            recipient, out = _capture(
                email_delivery.prompt_email_recipient,
    ...
    def test_helper_predicates(self):
        self.assertTrue(email_delivery.is_affirmative("YES"))
        self.assertFalse(email_delivery.is_affirmative("nope"))
        self.assertTrue(email_delivery.is_valid_email("a.b@c.co"))
        self.assertFalse(email_delivery.is_valid_email("a@b"))

    def test_recipient_round_trips_through_a_real_store_file(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = Path(tmp) / ".apppilot_recipient"
            with mock.patch.object(email_delivery, "_RECIPIENT_STORE", store):
                answers = iter(["y", "saved@example.com"])
                recipient = email_delivery.prompt_email_recipient(
                ...
                self.assertEqual(email_delivery._load_saved_recipient(), "saved@example.com")
    ...
    def test_corrupt_store_file_never_raises(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(email_delivery, "_RECIPIENT_STORE", store):
                ...
                self.assertIsNone(email_delivery._load_saved_recipient())
    ...
    def test_prompt_up_front_then_send_after_delivers_to_chosen_recipient(self):
        ...
        with mock.patch.object(email_delivery, "_load_saved_recipient", return_value=None), \
                mock.patch.object(email_delivery, "_save_recipient"):
        ...
        sent = email_delivery.send_suite_report(
