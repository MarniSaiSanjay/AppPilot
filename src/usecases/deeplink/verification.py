...
class LLMExpectationJudge:
    ...
    _SYSTEM_PROMPT = (
        ...
        "\"Researcher screen without any prompt\", \"Cowork screen with prompt "
        "\\\"<specific text>\\\"\", or an expected error/failure state) "
        ...
        "result. Judge the screen TYPE by its combined semantic evidence, not by "
        "one hardcoded label or selector.\n"
        "\n"
        "RULES:\n"
        ...
        "- A named destination is authoritative. If Expected Result names "
        "Researcher, Cowork, Chat, or another screen, the observed UI must be that "
        "same screen type. Researcher, Cowork, and generic Chat/Copilot are "
        "different destinations and must not match one another.\n"
        ...
        "agree.\n"
        "- 'without prompt' or 'without any prompt': the destination's "
        "composer/input must contain no prefilled prompt. Placeholder or hint text "
        "does not count as a prompt. A visible clickable placeholder such as "
        "'Message Copilot' is positive evidence that the Chat composer exists and "
        "is empty; do not require it to be exposed as an editable input node.\n"
        "- Normal bottom-navigation destinations such as Chat, Search, Library, "
        "and Notebooks can coexist on the Chat screen. Their presence does not by "
        "itself mean that a menu, drawer, or unrelated screen is open.\n"
