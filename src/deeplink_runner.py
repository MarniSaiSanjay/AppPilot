...
# Support both `python -m src.deeplink_runner` (package-relative) and running as
# a script / with src on sys.path (top-level), without changing behavior.
try:
    from .apppilot_agent import (  # noqa: E402
        MaestroExecutor,
...
def _default_excel_path() -> Path:
    return Path(__file__).resolve().parent.parent / "testcases" / "deeplinks" / (
        "deeplink_tests.xlsx"
    )
